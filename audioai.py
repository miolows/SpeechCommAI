...
class AudioAI():
    def __init__(self, num_classes, to_train, ai_dir):
        self.num_classes = num_classes
        if to_train:
            self.create_model()
            validation, testing, training = self.load_data("mfcc_Dataset")
            #hard coded num of epochs!!!
            self.train(10, self.prep_data(training), self.prep_data(validation), ai_dir)
            
        else:
            model_dir = os.path.join(ai_dir, self.num_classes)
